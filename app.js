...
app.get("/",(req,res)=>{
  ...
  Post.find({},(err,arr)=>{
    ...
    res.render("home",{data:homeStartingContent,posts:arr});
  });
...
app.post("/",(req,res)=>{
  Post.deleteOne({_id:req.body.delp},(err)=>{
    if(err){
      console.log(err);
    }
  });
...
app.get("/posts/:postName",(req,res)=>{
  ...
  Post.find({_id:req.params.postName},(err,arr)=>{
    if(arr==null){
      res.render("post",{title:"Error",data:"Post not found"});
    }
    else{
      res.render("post",{title:arr[0].title,data:arr[0].post});
    }
  });
